...
export default function EditBlog() {
  ...
  const submit = async () => {
    ...
    try {
      setSaving(true);
      await api.put(`/posts/${id}`, { title, content, coverPhoto, excerpt, category });
      navigate(`/blogs/${id}`);
    } catch (err) {
      console.log(err);
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <h1 className="text-4xl font-bold text-white mb-2">
              Edit <span className="text-cyan-400">Story</span>
            </h1>
            <p className="text-slate-400">Update your story content</p>
          </div>
          <button
